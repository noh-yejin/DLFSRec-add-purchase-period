...
class DLFSRecModel(nn.Module):
    def __init__(self, args):
        ...
        if self.args.period_add_like_cxt==True:
            self.side_mean_dense = nn.Linear(args.feature_size+1, args.attribute_hidden_size)
            self.side_cov_dense = nn.Linear(args.feature_size+1, args.attribute_hidden_size)
        else: 
    ...
    def forward(self, input_ids, input_context):
        ...
        mean_id_emb = self.item_mean_embeddings(input_ids)   #torch.Size([256, 50, 128]), 128=batch_size
        cov_id_emb = self.item_cov_embeddings(input_ids)   #torch.Size([256, 50, 128])
        input_attrs = self.args.items_feature[input_ids]  #torch.Size([256, 50, 9866]
        ...
        item_encoded_layers = self.item_encoder(mean_sequence_emb,  #[256,50,32]:[batch, max_len, hidden_size]
                                                cov_sequence_emb, #[256,50,32]:[batch, max_len, hidden_size]
                                                output_all_encoded_layers=True)
        sequence_mean_output, sequence_cov_output = item_encoded_layers[-1]
        # size->[256,50,32]:[batch, max_len, hidden_size]
