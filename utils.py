...
def get_data_dic(args): 
    if args.e_commerce==True: # args.e_commerce
        dat = pkl.load(open(f'{args.data_dir}{args.data_name}.dat', 'rb'))
    else:
    ...
    data['e_commerce']=True # for get_feats_vec
    ...
    if args.period_add_like_cxt == True:
        data['user_seq_period']=[]
        for u in dat['item_period']:
            data['user_seq_period'].append(dat['item_period'][u]) # period add 
...
def get_feats_vec(feats, args):
    ...
    #check feats 
    feats= torch.tensor(feats) #e-commerce:torch.Size([48443, 3]) <->grocery:torch.Size([41281, 12])
    feat_category = torch.zeros(feats.size(0), args['n_categories']) # e-commerce: torch.Size([48443, 997]) <->grocery: torch.Size([41281, 1004])
    # if args['e_commerce'] == True: 

    #     category_vec = feat_category.scatter_(index=feats[:,:1].long(), value=1, dim=-1)
    #     feat_brand = torch.zeros(feats.size(0), args['n_brands'])
    #     brand_vec = feat_brand.scatter_(index=feats[:, 1:].long(), value=1, dim=-1)
    #     vec = torch.cat(( category_vec[:, 1:], brand_vec[:, 1:]), dim=1)
    # else: 
